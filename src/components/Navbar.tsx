...
import { BriefcaseBusiness as Linkedin, FileDown, GitFork as Github, Menu, X } from 'lucide-react';
...
export function Navbar({ theme, toggleTheme }: { theme: 'dark' | 'light'; toggleTheme: () => void }) {
  ...
  return <header className="navbar"><div className="nav-inner"><button className="brand" onClick={() => go('Home')} aria-label="Go home"><span>{personalInfo.shortName}</span><strong>{personalInfo.name}</strong></button>
    ...
    <div className="nav-actions">{personalInfo.github && <a className="nav-social" href={personalInfo.github} aria-label="GitHub"><Github size={18}/></a>}{personalInfo.linkedin && <a className="nav-social" href={personalInfo.linkedin} aria-label="LinkedIn"><Linkedin size={18}/></a>}<ThemeToggle theme={theme} toggle={toggleTheme}/><a className="button compact desktop-resume" href={personalInfo.resume} download><FileDown size={16}/> Resume</a><button className="menu-button" onClick={() => setOpen(v => !v)} aria-expanded={open} aria-label="Toggle menu">{open ? <X/> : <Menu/>}</button></div>
  </div></header>;
}
